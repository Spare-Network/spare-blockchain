...
def check_keys(new_root: Path) -> None:
    ...
    if "spare_target_address" not in config["farmer"]:
        print(f"Setting the spare destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "spare_target_address" not in config["pool"]:
        print(f"Setting the spare destination address for coinbase reward to {all_targets[0]}")
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if spare_init(root_path) == 0 and root_path.exists():

...
function loadConfig(version) {
  try {
    ...
    // 1) SPARE_ROOT environment variable
    // 2) version passed in and determined by the `spare version` call

    // check if SPARE_ROOT is set. it overrides everything else
    const config_root_dir =
      'SPARE_ROOT' in process.env
        ? process.env.SPARE_ROOT
        : path.join(os.homedir(), '.spare', version);

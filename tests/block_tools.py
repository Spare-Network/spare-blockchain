...
from spare.util.condition_tools import ConditionOpcode, conditions_by_opcode
...
class BlockTools:
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "spare")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("SPARE_ROOT", "~/.spare-blockchain/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
